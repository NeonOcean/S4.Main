...
from objects import definition_manager, script_object, definition
...
class EverythingRegistrationInformation(RegistrationInformation):
	def GetRelevantObjectTypes (self) -> typing.Set[str]:
		"""
		Get a list of the types of game object that the interaction should be attached to.
		"""
...
class ByObjectInstanceIDRegistrationInformation(RegistrationInformation):
	FACTORY_TUNABLES = {
		"InstanceIDs": tunable.TunableList(
			description = "The instance id of any game objects that the interaction should be attached to.",
...
class ByDefinitionInstanceIDRegistrationInformation(RegistrationInformation):
	FACTORY_TUNABLES = {
		"InstanceIDs": tunable.TunableList(
			description = "The instance id of any game object definition that the interaction should be attached to. The interaction would be attached to the definition's game object, it may be simpler to just designate the game object its self.",
			tunable = tunable.Tunable(tunable_type = int, default = 0)
		)
	}
...
class ByTypeRegistrationInformation(RegistrationInformation):
	FACTORY_TUNABLES = {
		"Types": tunable.TunableList(
			description = "The types of game object that the interaction should be attached to. A corresponding type determiner python function needs to also exist and be setup for object registration to occur.",
...
class RegistrationInformationVariant(tunable.TunableVariant):
	def __init__ (self, *args, **kwargs):
		super().__init__(
			*args,
			everything = EverythingRegistrationInformation.TunableFactory(),
			byObjectInstanceID = ByObjectInstanceIDRegistrationInformation.TunableFactory(),
			byDefinitionInstanceID = ByDefinitionInstanceIDRegistrationInformation.TunableFactory(),
			byType = ByTypeRegistrationInformation.TunableFactory(),
			**kwargs
		)
...
class RegistrationExtension:
	...
	def RegisterToObjects (cls) -> None:
		...
		objectTypeOrganizer = cls._GetObjectTypeOrganizer()  # type: typing.Type[RegistrationTypes.ObjectTypeOrganizer]
		objectsByType = objectTypeOrganizer.GetObjectsByType()  # type: typing.Dict[str, typing.Set[typing.Type[script_object.ScriptObject]]]
		...
		for registrationInformation in cls.ObjectRegistrationInformation:  # type: RegistrationInformation
			...
			for relevantDefinitionInstanceID in registrationInformation.GetRelevantDefinitionInstanceIDs():  # type: int
				matchingDefinition = definitionManager.get(relevantDefinitionInstanceID)  # type: typing.Optional[definition.Definition]
			...
			for relevantObjectType in registrationInformation.GetRelevantObjectTypes():  # type: str
				matchingObjects = objectsByType.get(relevantObjectType, None)  # type: typing.Optional[typing.Set[typing.Type[script_object.ScriptObject]]]
	...
	@classmethod
	def _GetObjectTypeOrganizer (cls) -> typing.Type[RegistrationTypes.ObjectTypeOrganizer]:
