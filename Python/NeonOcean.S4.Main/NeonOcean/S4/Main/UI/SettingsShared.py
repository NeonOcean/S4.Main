...
class SettingBranchWrapper(SettingWrapper):
	def __init__ (self, setting: typing.Type[AbstractSettings.SettingBranchedAbstract], branch: str):
		"""
	...
	@property
	def Setting (self) -> typing.Type[AbstractSettings.SettingBranchedAbstract]:
		return self._setting
	...
	def IsHidden (self) -> bool:
		return self.Setting.IsHidden(self.Branch)

	def Get (self, ignoreOverride: bool = False) -> typing.Any:
		return self.Setting.Get(self.Branch, ignoreOverride = ignoreOverride)

	def GetDefault (self) -> typing.Any:
		return self.Setting.Default

	def Set (self, value: typing.Any, autoSave: bool = True, autoUpdate: bool = True) -> None:
		return self.Setting.Set(self.Branch, value, autoSave = autoSave, autoUpdate = autoUpdate)

	def Reset (self, autoSave: bool = True, autoUpdate: bool = True) -> None:
		self.Setting.Reset(branch = self.Branch, autoSave = autoSave, autoUpdate = autoUpdate)

	def IsOverridden (self) -> bool:
		return self.Setting.IsOverridden(self.Branch)

	def GetOverrideValue (self) -> typing.Any:
		activeOverrideIdentifier = self.Setting.GetActiveOverrideIdentifier(self.Branch)  # type: str
		return self.Setting.GetOverrideValue(activeOverrideIdentifier)
	...
	def CanShowDialog (self) -> bool:
		return self.Setting.CanShowDialog(self.Branch)

	def ShowDialog (self, returnCallback: typing.Callable[[], None] = None) -> None:
		if not isinstance(returnCallback, typing.Callable) and returnCallback is not None:
			raise Exceptions.IncorrectTypeException(returnCallback, "returnCallback", ("Callable", None))

		return self.Setting.ShowDialog(self.Branch, returnCallback = returnCallback)
...
class SettingsSystemBranchWrapper(SettingSystemWrapper):
	def __init__ (self, settingSystem: typing.Any,
		...
		for settingIndex in range(len(settings)):  # type: int
			setting = settings[settingIndex]  # type: SettingBranchWrapper

			if not isinstance(setting, SettingBranchWrapper):
				raise Exceptions.IncorrectTypeException(setting, "settings[%s]" % settingIndex, (SettingBranchWrapper,))

		self._settings = settings  # type: typing.List[SettingBranchWrapper]
		self._save = save  # type: typing.Callable[[], None]
		self._update = update  # type: typing.Callable[[], None]

	@property
	def Settings (self) -> typing.List[SettingBranchWrapper]:
		return self._settings
